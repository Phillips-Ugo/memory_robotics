...
class World:
    ...
    def sample_task(self, p_touch: float = 0.7) -> Task:
        ...
        if self.rng.random() < p_touch:
            ...
            if r < 2 / 3:
                obj = self.rng.choice([o for o in self.objects if o != self.props.heavy_object])
                return Task(str(obj), self.props.sticky_drawer)
            drawer = self.rng.choice([d for d in self.drawers if d != self.props.sticky_drawer])
