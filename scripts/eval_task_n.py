"""Evaluate any RoboMemArena task (by --task-id) with a custom policy adapter; same protocol as eval_task1_only.py.
Copy into vendor/RoboMemArena/evaluation_benchmark/scripts/.
"""
...
def main(args: Args) -> dict:
    ...
    stage_checks = [(spec.name, lambda env, spec=spec: spec.check_fn(env, {}, 0)) for spec in stage_eval._task_specs(args.task_id)]
    return ec.run_eval(
